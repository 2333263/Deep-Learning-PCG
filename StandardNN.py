...
def trainModel(trainingData,trainingTargets,testingData,testingTargets):
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(30*45, activation='relu'),
   ...
   # print(history)
    ...
    np.save("NPYs/StandardNN/CorrectChanges.npy",correctChanges)
    np.save("NPYs/StandardNN/Loss.npy",history.history['loss'])
    np.save("NPYs/StandardNN/ValLoss.npy",history.history['val_loss'])
    plt.plot(history.history['loss'],label="Training Loss")
    plt.plot(history.history['val_loss'],label="Validation Loss")
    plt.legend()
    plt.savefig("Graphs/StandardNN/Loss.png")
    model.save("Models/StandardNN",save_format="tf")
