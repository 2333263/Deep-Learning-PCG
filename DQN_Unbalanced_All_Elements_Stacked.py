...
stacks the entire level along with the 12x12 window as the input
'''
...
def trainModel(trainingData,trainingTargets,testingData,testingTargets):
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(30,45,2)),
   ...
   # print(history)
    ...
    with open("NPYs/DQN_Unbalanced_All_Elements_Stacked/Info.json","w") as f:
        json.dump(information,f)
        f.close()
    np.save("NPYs/DQN_Unbalanced_All_Elements_Stacked/Loss.npy",history.history['loss'])
    np.save("NPYs/DQN_Unbalanced_All_Elements_Stacked/ValLoss.npy",history.history['val_loss'])
    plt.plot(history.history['loss'],label="Training Loss")
    plt.plot(history.history['val_loss'],label="Validation Loss")
    plt.legend()
    plt.savefig("Graphs/DQN_Unbalanced_All_Elements_Stacked/Loss.png")
    model.save("Models/DQN_Unbalanced_All_Elements_Stacked",save_format="tf")
